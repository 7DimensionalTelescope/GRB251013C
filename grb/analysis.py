...
class Analyzer(Fitter):
    ...
    def get_posterior_predictive_curve(self, x_data, model=None, param_indices=None, percentiles=[16, 50, 84]):
        if self.results is None:
            raise ValueError("No results found, Analyzer.run() must be called first")

        if model is None:
            model = self.model
            param_indices = range(len(self.params))
        elif param_indices is None:
            raise ValueError("param_indices must be provided if model is provided")

        sed_samples = []
        for i in range(len(self.results["samples"])):
            data = model(x_data, *self.results["samples"][i][param_indices])
            sed_samples.append(data)
        percentiles = np.percentile(np.array(sed_samples).T, percentiles, axis=1)
        dataset = np.array([x_data] + list(percentiles)).T
        return dataset
