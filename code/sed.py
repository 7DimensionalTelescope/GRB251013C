...
def plot_sed_from_samples(x_data, samples, ax=None, x_unit="Hz", y_unit="mJy", add_median=False, **kwargs):
    if ax is None:
        ax = plt.gca()

    sed_samples = []
    for i in range(len(samples)):
        data = power_law(x_data, *samples[i][:2], x0=1e15)
        sed_samples.append(data)
    percentiles = np.percentile(np.array(sed_samples).T, [16, 50, 84], axis=1)
